...
function FacturacionElectronica() {
  ...
  const guardarConfiguracion = async () => {
    try {
      setGuardando(true);
      setError('');
      await api.put(`/negocios/${negocioId}`, { configuracion: config });
  ...
  const generarCertificados = async () => {
    ...
    try {
      ...
      const res = await api.post(`/negocios/${negocioId}/arca/generar-certificados`, {
  ...
  const subirCertificado = async () => {
    ...
    try {
      ...
      await api.post(`/negocios/${negocioId}/arca/subir-certificado`, formData, {
  ...
  const eliminarCertificado = async (id) => {
    ...
    try {
      await api.delete(`/negocios/${negocioId}/arca/certificados/${id}`);
  ...
  const emitirComprobante = async () => {
    ...
    try {
      ...
      const res = await api.post(`/negocios/${negocioId}/arca/emitir`, {
        ...formComprobante,
  ...
  const descargarArchivo = async (tipo, filename) => {
    try {
      const res = await api.get(`/negocios/${negocioId}/arca/descargar/${tipo}/${filename}`, {
        responseType: 'blob'
      });
  ...
  const certificadoActivo = Array.isArray(certificados) ? certificados.find(c => c.activo) : null;
              ...
              <Toggle
                ...
                onChange={async (v) => {
                  ...
                  try {
                    ...
                    const res = await api.put(`/negocios/${negocioId}`, { configuracion: configToSend });
                    // Actualizar con la respuesta del servidor
                    setConfig(res.data.negocio?.configuracion || res.data.configuracion);
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          const res = await api.post(`/negocios/${negocioId}/arca/test-conexion`);
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={async () => {
                  try {
                    const res = await api.put(`/negocios/${negocioId}`, {
                      configuracion: {
                        ...config,
