// =============================================
// SERVICIO: Backup automatico de la base de datos
// Hace un dump diario de PostgreSQL con pg_dump y rota los archivos viejos.
// Configuracion por .env (opcional):
//   BACKUP_ENABLED=true|false   (default: true)
//   BACKUP_HORA=3               (hora del dia, default 3 AM)
...
function hacerBackup() {
  return new Promise((resolve, reject) => {
    ...
    const stamp = fecha.toISOString().replace(/[:T]/g, '-').slice(0, 16); // 2026-07-03-03-30
    const nombre = `backup_${process.env.DB_NAME || 'burgerpos'}_${stamp}.dump`;
    ...
    proceso.on('error', (err) => {
      // pg_dump no instalado / no esta en el PATH
      reject(new Error(`No se pudo ejecutar pg_dump: ${err.message}`));
    });

    proceso.on('close', (code) => {
      if (code !== 0) {
        try { if (fs.existsSync(destino)) fs.unlinkSync(destino); } catch { /* ignore */ }
...
/** Borra los backups mas viejos, conservando los ultimos N */
function rotarBackups() {
  try {
    ...
    const archivos = fs.readdirSync(BACKUP_DIR)
      ...
      .sort()       // el nombre incluye la fecha, ordena cronologico
      .reverse();   // mas nuevos primero
...
/** Lista los backups existentes (mas nuevos primero) */
